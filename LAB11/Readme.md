...
### Шаг 1. Реализация различных оптимизаций на каждом маршрутизаторе.
 ...
 - На R1 настройте статический маршрут по умолчанию, который использует интерфейс Loopback 1 в качестве интерфейса выхода. Затем рследует распространить маршрут по умолчанию в OSPF иобратить внимание на сообщение консоли после установки маршрута по умолчанию.
 ...
 - Изменение базовой пропускной способности для маршрутизаторов. После этой настройки нужно перезапустить OSPF с помощью команды **clear ip ospf process** и обратить внимание на сообщение консоли после установки новой опорной полосы пропускания.
### Шаг 2. Проверка того, что оптимизация OSPFv2 реализовалась.
 - a. Выполнение команды **show ip ospf interface g0/0/1** на R1 и проверка того, что приоритет интерфейса установлен равным 50, а временные интервалы — Hello 30, Dead 120, тип сети по умолчанию — Broadcast
 - b. На R1 выполните команду show ip route ospf, чтобы убедиться, что сеть R2 Loopback1 присутствует в таблице маршрутизации. Обратите внимание на разницу в метрике между этим выходным и предыдущим выходным. Также Следует обратить внимание на то, что маска теперь составляет 24 бита, в отличие от 32 битов, ранее объявленных.
 - c. Ввод команды **show ip route ospf** на маршрутизаторе R2. Единственная информация о маршруте OSPF должна быть распространяемый по умолчанию маршрут R1.
 - d. Запуск Ping до адреса интерфейса R1 Loopback 1 из R2. Выполнение команды ping должно быть успешным.
