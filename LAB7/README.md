...
![](https://github.com/IvShikov/OtusLab/blob/main/LAB7/Lab7_P2_S4_Scheme%20to%20fulfill%20new.jpg)

С учётом выходных данных, поступающих с коммутаторов, выяснили, что коммутатор S1 является корневым мостом. 
...
Корневыми портами на коммутаторах являются порт f0/4 свитча S3 и f0/2 свитча S2.
Назначенными портами на коммутаторе являются порт f0/4 и f0/2 свитча S1 и f0/4 свитча S2.
В качестве альтернативного отображается порт f0/2 свитча S3 и он в настоящее время заблокирован.
Протокол spanning-tree выбрал этот порт в качестве невыделенного (заблокированного) порта потому, что алгоритм spanning-tree использует корневой порт как референсную точку, а затем - определяет, какие порты заблокировать на основании стоимости пути. Если стоимости равны, процесс сравнивает BID. Наиболее низкие значения являются предпочтительными. Если стоимости пути через оба свитча равны, алгоритм выбирает путь до свитча с наименьшим BID и блокирует порт f0/2 свитча S3 с бОльшим BID.
#
### Часть 3. Наблюдение за процессом выбора протоколом STP порта, исходя из стоимости портов
...
При текущей конфигурации только один коммутатор может содержать заблокированный протоколом STP порт. Выполнили команду show spanning-tree на обоих коммутаторах некорневого моста. У нас протокол spanning-tree блокирует порт F0/2 на коммутаторе с самым высоким идентификатором BID (S3).

![](https://github.com/IvShikov/OtusLab/blob/main/LAB7/Lab7_BLK%20port%20definition.JPG)
...
Повторно выполним команду show spanning-tree на обоих коммутаторах некорневого моста. Ранее заблокированный порт (S3 – F0/2) теперь является назначенным портом и протокол spanning-tree теперь блокирует порт на другом коммутаторе некорневого моста (S3 – F0/4). ПЕРЕДЕЛАТЬ!!!
...
![](https://github.com/IvShikov/OtusLab/blob/main/LAB7/Lab7_P3_S2-3.JPG)
![](https://github.com/IvShikov/OtusLab/blob/main/LAB7/Lab7_P3_S2-3b.JPG)

Протокол spanning-tree заменяет ранее заблокированный порт на назначенный порт и блокирует порт, который был назначенным портом на другом коммутаторе потому, что сначала STP смотрит на стоимость пути, а порт с более низкой стоимостью будет всегда более предпочтительным, чем порт с более высокой стоимостью.   
...
![](https://github.com/IvShikov/OtusLab/blob/main/LAB7/Lab7_P3_S4.JPG)
...
![](https://github.com/IvShikov/OtusLab/blob/main/LAB7/Lab7_P4_S1.JPG)

![](https://github.com/IvShikov/OtusLab/blob/main/LAB7/Lab7_P4_S2.JPG)

![](https://github.com/IvShikov/OtusLab/blob/main/LAB7/Lab7_P4_S3.JPG)

Протокол STP в качестве порта корневого моста на коммутаторе S3 некорневого моста выбрал порт f0/3, на коммутаторе S2 некорневого моста - порт f0/1.
Протокол STP выбрал эти порты в качестве портов корневого моста на этих коммутаторах потому, что значение порта по умолчанию - 128 и STP использовал номер порта, чтобы разорвать связь. Он выбрал в качестве корневого порта порт с самым низким значением порта, а также - заблокировал порт с более высоким значением и резервным путём к корневому мосту.

#### Вопросы для повторения:
##### 1.	Чтобы определить выбор порта, протокол STP использует стоимость пути первой после выбора корневого моста. Он выбирает путь с минимальной стоимостью.
##### 2.	Если первое значение на двух портах одинаково, протокол STP при выборе порта будет использвать следующим значение BID (наименьшее).
##### 3.	Если оба значения на двух портах равны, следующее значение, которое использует протокол STP при выборе порта, будет самое низкое значение суммы приоритета порта и номера порта.
