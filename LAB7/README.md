...
Протокол STP выбрал эти порты в качестве портов корневого моста на этих коммутаторах потому, что значение порта по умолчанию - 128 и STP использовал номер порта, чтобы разорвать связь. Он выбрал в качестве корневого порта порт с самым низким значением порта и заблокировал порт с более высоким значением с альтернативным путём к корневому порту.
