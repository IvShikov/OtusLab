...
Повторно выполним команду show spanning-tree на обоих коммутаторах некорневого моста. Ранее заблокированный порт (S3 – F0/2) теперь является назначенным портом и протокол spanning-tree теперь блокирует порт на другом коммутаторе некорневого моста (S2 – F0/4). 

![](https://github.com/IvShikov/OtusLab/blob/main/LAB7/Lab7_Change%20BLK%20nonroot.JPG)
...
![](https://github.com/IvShikov/OtusLab/blob/main/LAB7/Lab7_Return%2019.JPG)
...
![](https://github.com/IvShikov/OtusLab/blob/main/LAB7/Lab7_All%20ports%20up.JPG)
...
Протокол STP выбрал эти порты в качестве портов корневого моста на этих коммутаторах потому, что значение порта по умолчанию - 128 и STP использовал номер порта, чтобы разорвать связь. Он выбрал в качестве корневых портов оные с самым низким значением порта, а также - заблокировал порты с более высоким значением и резервным путём к корневому мосту.
