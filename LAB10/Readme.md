...
![](https://github.com/IvShikov/OtusLab/blob/main/LAB10/Lab10_P3S6_S1.JPG)

![](https://github.com/IvShikov/OtusLab/blob/main/LAB10/Lab10_P3S6_S2.JPG)
...
## **Вопросы для повторения:**
 - С точки зрения безопасности порта на S2, почему нет значения таймера для оставшегося возраста в минутах, когда было сконфигурировано динамическое обучение - sticky?- Потому что свитч не поддерживает значение оставшегося времени безопасности порта для адресов, когда было сконфигурировано динамическое обучение - sticky
 - Что касается безопасности порта на S2, если вы загружаете скрипт текущей конфигурации на S2, почему порт 18 на PC-B никогда не получит IP-адрес через DHCP?- Потому что порт безопасности сконфигурирован только для 2 МАС-адресов, а у порта 18 имеется два sticky МАС-адреса, связанных с портом, с защитой от нарушений, который никогда не отправит другим устройствам лог-сообщение или инкрементирует счётчик числа нарушений. 
 - Что касается безопасности порта, в чём разница между типом абсолютного устаревания и типом устаревание по неактивности? - Если задано время неактивности, безопасные адреса на порту будут удалены только в отсутствии передачи данных от защищённых источников адресов за заданный промежуток времени; если задано абсолютное время, все безопасные адреса устаревают именно по истечении указанного промежутка времени.  
