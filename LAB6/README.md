...
#### Вопрос: Если G0/0/1 на R1 будет отключен, порт f0/5 коммутатора S1 не будет отображён.
...
#### В результатах отображаются промежуточные IP-адреса роутера R1, который является для PC-B гейтвеем (192.168.30.1) и компьютера PC-A (192.168.20.3), поскольку компьютер PC-B расположен в отличной от PC-А сети и обращается к роутеру R1.
